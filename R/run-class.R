...
validate_run <- function(x) {
  ...
  assertList(x)
  for (param in names(x)) assertChoice(param, c("id", "created_at", "assistant_id", "thread_id", "status", "started_at", "expires_at", "cancelled_at", "failed_at", "completed_at", "last_error", "model", "instructions", "tools", "file_ids", "metadata", "usage", "temperature", "required_action"))
  qassert(x$id, "S1")
