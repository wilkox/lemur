...
validate_assistant_tool <- function(x) {
  ...
  assertChoice(params$type, c("code_interpreter", "retrieval", "function"))
...
#'
#' @param type The type of tool, one of "retrieval" (the default), "code_interpreter", or "function"
#' @export
assistant_tool <- function(type = "retrieval") {
  x <- list(type = type)
